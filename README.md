...
> 본 문서는 **전체 파이프라인 이해**를 위한 요약이며, 세부 구조·차원·알고리즘은 연구 보호를 위해 공개하지 않습니다.
> 본 저장소는 기술 보호를 위해 모델 파라미터 및 주요 구현 요소를 비공개 처리한 버전입니다. 프로젝트의 실제 전체 코드는 정상적으로 실행되며, 검증을 위해 해당 완전한 코드를 교수님께 별도로 전달드릴 예정입니다.
